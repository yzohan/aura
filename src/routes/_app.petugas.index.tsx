...
export const Route = createFileRoute("/_app/petugas/")({
  head: () => ({ meta: [{ title: "Work Order Petugas" }] }),
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const { data, error } = await supabase
        .from("work_orders")
        .select("id, report_id, notes, proof_photo_url, completed_at, created_at, report:reports(id, name, no_hp, detail_laporan, category, status_pelaporan, kategori_pelaporan, address, latitude, longitude)")
...
function WorkOrderCard({ wo, userId }: { wo: WorkOrderItem; userId: string }) {
  ...
  const statusKey = (r.status_pelaporan === "progress" ? "in_progress" : r.status_pelaporan) as keyof typeof STATUS_LABEL;
  const urgencyKey = (r.kategori_pelaporan === "ringan" ? "low" : r.kategori_pelaporan) as keyof typeof URGENCY_LABEL;
  ...
  const markDone = async () => {
    ...
    const [{ error: woErr }, { error: rErr }] = await Promise.all([
      ...
      supabase.from("reports").update({ status_pelaporan: "resolved" }).eq("id", r.id),
    ]);
    setSubmitting(false);
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <h3 className="mt-1 text-lg font-semibold flex items-center gap-2">
            ...
            <span>Laporan dari {r.name} ({r.no_hp})</span>
          </h3>
          <p className="text-xs text-muted-foreground">{CATEGORY_LABEL[r.category]}</p>
        </div>
        <div className="flex flex-col items-end gap-1">
          <Badge variant="outline" className={STATUS_TONE[statusKey] || "bg-secondary text-secondary-foreground"}>
            {statusKey === "in_progress" ? "Dikerjakan" : (STATUS_LABEL[statusKey] || r.status_pelaporan)}
          </Badge>
          <Badge variant="outline" className={URGENCY_TONE[urgencyKey] || "bg-secondary text-secondary-foreground"}>
            {urgencyKey === "low" ? "Ringan" : (URGENCY_LABEL[urgencyKey] || r.kategori_pelaporan)}
          </Badge>
        </div>
      </div>

      <p className="mt-3 text-sm text-muted-foreground">{r.detail_laporan}</p>
